...
## 2026-09-06 (Accessibility Health & Recovery V1 emulator phase — APPAUSE-20260906-1459)
- Added the bounded `AccessibilityHealthState` contract. It keeps system evidence (`ENABLED`, `DISABLED`, `UNKNOWN`) separate from process evidence (`CONNECTED`, `DISCONNECTED`, `UNKNOWN`); only `ENABLED + CONNECTED` is healthy. A process is marked `DISCONNECTED` only after the observed connected service instance is destroyed. Fresh process state remains `UNKNOWN` until `onServiceConnected`.
- Centralized health derivation and observation in `AccessibilityHealthChecker`/`AccessibilityHealthPolicy`, and made Home, Permissions, About, and Onboarding observe the same state. The process `StateFlow` recomputes health when the service connects or is destroyed; resume still re-reads the system setting. UNKNOWN renders a neutral unverified/checking path, while disabled and proven disconnected have recovery actions to normal Android Accessibility Settings.
- Added six deterministic JVM policy tests covering healthy, disabled-wins, unknown/fail-closed, connected transition, and observed-destroy transition. `:app:testDebugUnitTest` passed with 134 tests and 0 failures; `:app:assembleDebug` passed.
- Emulator-only validation used API 34 `Appause_P0_Nav_API34_Temp`, serial `emulator-5554`, model `sdk_gphone64_x86_64`, with `ro.kernel.qemu=1`. Data-preserving `install -r`, normal Settings UI Stop/Allow, Back/resume refresh, reboot-preserved enabled state, Bound service, and post-reboot Appause launch all passed. Disabled return showed the red Accessibility recovery card; enabled return and post-reboot launch omitted the missing-permission item. No direct secure-setting write or `uiautomator` was used.
- Healthy interception against a configured target was NOT TESTED because this emulator had no configured group and creating one would change user data. Physical Xiaomi serial `6036d5b` received no install/start/settings command. No commit, push, merge, tag, release, or deploy occurred.
- Independent review follow-up found and fixed a narrow lifecycle race: health snapshots now read the process `StateFlow` directly instead of allowing the static instance reference to override a just-emitted `DISCONNECTED` state. System-state setting reads now convert any ordinary read exception to `UNKNOWN`, preserving fail-closed observation. The focused JVM tests and `assembleDebug` were rerun successfully; the rebuilt APK was installed with `-r` on `emulator-5554`, where disabled settings and the disabled recovery UI remained correct. No blocking review finding remains.

## 2026-09-06 (Release-engineering audit: interception protocol, release checklist, test gap analysis — docs-only)
- Produced three read-only documentation deliverables under `docs/`, authorized by the user as items 1–3 of the maintainability campaign; no source, build config, or test code was touched.
- `docs/INTERCEPTION_PROTOCOL.md`: documents the real interception behavior at `main` @ `461ee33` — component map, dual input sources (event stream + 1.5 s poller), the two-stage pure decision pipeline with exact filter order, Home/Recents transition confirmation (750 ms settle, watchdog fail-open), the pause-guard watchdog semantics (1.5 s grace / 30 s hard cap), the 2032 → 2038 → PauseActivity presentation chain, the session lifecycle state machine (Mermaid), the three coexisting session concepts (runtime bypass vs SessionState marker vs persisted TemporaryPass), an AGENTS.md invariant-to-enforcement-point table, and a prioritized risk register (R1 static-state cluster, R2 side-effectful `pauseShown` reads with 4+ hand-inlined `pausePresentationActive` expressions, R3 unsynchronized bypass set, R4–R7 lower-priority items).
- `docs/RELEASE_CHECKLIST.md`: a per-release gate covering version/build config, secrets and privacy checks (including the `.dev.vars` gitignore regression fixed this session), behavior confirmation items, test gates with the emulator-vs-physical reporting rule, debug/release isolation, packaging via `scripts/make_release.py`, and publication materials.
- `docs/TEST_GAP_ANALYSIS.md`: maps the 134-test baseline against the protocol. Highest gaps: G1 service orchestration layer (decision→effect mapping) has zero tests; G2 pause-guard watchdog untestable in current form; G3 Pro redeem failure modes (user priority #1); G4 Room migrations (1 test); G5 re-remind timing math; G6 leave-timer lifecycle; G7 ViewModel coverage 3/9. Proposes a six-step execution order.
- Verification: docs-only change; no build or test run was required or performed. `git status` after the turn contains only these new files plus the pre-existing `index.html` modification. No commit, push, tag, release, or deploy was performed.

## 2026-09-06 (Engineering-review batch 1: docs alignment + CI + P0-1/P0-2/P0-3/P1-7 — APPAUSE-20260906-batch1)
- User-authorized batch from the merged engineering backlog (`docs/ENGINEERING_REVIEW.md`): items ① ② ③ ④⑤ ⑦.
- ① Added `docs/ENGINEERING_REVIEW.md` (full reviewed backlog with statuses, WONTFIX list, verified-good list, execution order). Aligned `docs/TEST_GAP_ANALYSIS.md` G1 with the newly found event-concurrency gap (P1-4) and appended risks R8 (no serialization of `handleForegroundChange`) and R9 (PauseActivity fallback: unconditional AlarmManager scheduling + stale-intent via missing `onNewIntent`; countdown-reset claim corrected — `launchMode="singleInstance"` re-fronts without recreation) to `docs/INTERCEPTION_PROTOCOL.md` §9.
- ② Added `.github/workflows/ci.yml`: push(main)/PR runs `testDebugUnitTest` + `assembleDebug` on temurin JDK 17, uploads test results on failure. Inactive until pushed.
- ③ (P0-1) `PauseActivity` now loads recommended apps from `repository.recommendedApps` (same DataStore source as OverlayManager's overlay path) instead of the deprecated `getLearningGroupPackageNames()`. Deleted the deprecated repository method and the `AppGroupDao.getLearningGroupPackageNames()` query. `AppGroup.TYPE_LEARNING` and its "learning groups are never intercepted" check in `findGroupForPackage` are intentionally preserved.
- ④ (P0-3) `HomeViewModel`: replaced the constructor-era `startOfToday` constant with a `_startOfToday` StateFlow; `proceededToday`/`cancelledToday` now `flatMapLatest` off it and `refreshServiceStatus()` (fires on every ON_RESUME) refreshes the window, so the "today" counters roll over at midnight instead of staying on the process-start day.
- ⑤ (P0-2) `HomeViewModel`: removed the manual `_sortedGroups` snapshot (which could be written from an empty `groups.value` during the `WhileSubscribed(5000)` gap and never recomputed). `sortedGroups` is now a `combine(groups, _interceptionCounts)` derived StateFlow. The promised `HomeViewModelTest` was NOT added in this batch — the ViewModel's `AppauseApp` cast needs a Robolectric harness with a custom test Application; scheduled with TEST_GAP_ANALYSIS G7a.
...
## 2026-09-06 (Batch: B1 dead-strategy cleanup + record retention, B2 test hardening, landing-page variants — user-authorized)
- B1: `OverlayPresentationPolicy.initialPath` converged to a constant (dead Xiaomi/canDrawOverlays params removed); `alternatePathAfterFailure` replaced by `shouldRetryWith2038AfterFailure(isXiaomiApi36OrLater)`; `OverlayWindowPolicy.flags()` parameterless; the unused `Settings.canDrawOverlays` query deleted from OverlayManager. `AppGroupRepository.deleteOldLaunchRecords()` (365-day retention, aligned with the stats window) is invoked from `AppauseApp.onCreate` on GlobalScope(IO), exceptions logged not raised. Dead code removed: `ForegroundChecker.wasResumedRecently`, the write-only `sessionStart` map, the unused `pauseTargetPackage` parameter of `HomeTransitionPolicy.shouldConfirm`/`shouldConfirmSystemUiHome`, and the unreachable `cancelLeaveTimer` line in `startLeaveTimer`. Stale comments fixed (GroupEditorViewModel clamp note, AppLaunchDao "v1" header).
- B2: pause-guard watchdog decision core extracted to pure `PauseGuardPolicy.evaluate` (exact historical boundaries: max-hold `> 30_000 ms`, grace `< 1_500 ms`), `pauseShown` getter rewired with unchanged read points and side effects. `LicenseVerifier.verify` now fails closed (returns null) on malformed segments, matching its documented contract; callers already wrapped it in runCatching so end-to-end behavior is unchanged. Tests: new `PauseGuardPolicyTest` (8 boundary cases) and 6 fail-closed `LicenseVerifierTest` cases.
- Verification: `assembleDebug` + `testDebugUnitTest` PASS with 147 tests (baseline 134, net +13). Emulator regression on emulator-5554: refactored build reinstalled `-r`, grouped-target launch still produced a real `type=2032` pause screen with the recommended-app row; overlay dismissed cleanly. Earlier in this session the full interception smoke (pause → Cancel → re-open → Continue → within-window Resume → 3-min re-arm) also passed on this AVD; see the TEST_REPORT.md entries.
- Landing pages: new `landing/` gallery with four variants — zh-a (light minimal), zh-b (dark marketing), en-a (minimal light), en-b (bold dark) — plus a gallery index; Chinese pages reference `images/screenshots/zh/` captures and English pages `images/screenshots/en/`, per the user's language-matching requirement. The pre-existing root `index.html` English redesign remains uncommitted as-is (user: keep for later evaluation). No selection has been made yet.
- Physical Xiaomi untouched throughout. Commits and push were performed under the user's standing authorization for this batch.

## 2026-09-16 (Duplicate countdown / repeated interception regression fix)
- Root cause: the 30-second PauseGuard hard cap could release the logical guard while a primary `TYPE_ACCESSIBILITY_OVERLAY` (2032) was still attached. Re-entering the same target could then create a second interception/countdown while the first overlay still existed.
- Minimal shared fix: `OverlayManager` now exposes whether the attached overlay can be hidden by the target. `PauseGuardPolicy` keeps the guard while PauseActivity is visible or while a non-hideable 2032 overlay remains attached; the 30-second hard cap remains only for the hideable 2038 fallback. Existing stale/no-presentation grace behavior is unchanged.
- Focused `PauseGuardPolicyTest` regression was red before the fix and PASS after it. `assembleDebug` and `git diff --check` passed for the implementation.
- Emulator-only validation on API 37 `Medium_Phone` (`emulator-5554`) passed. Valid S11 held the 2032 overlay beyond 30 seconds without watchdog release, then explicit app-leave semantics dismissed/abandoned it cleanly. Evidence: `scripts/stress/evidence/run-20260916-232249`.
- Representative human-like stress scenarios S4/S5/S6/S8/S9/S12 all completed. Analyzer on `scripts/stress/evidence/run-20260916-232559` parsed 12 INTERCEPT and 12 overlay show/dismiss cycles and reported no duplicate interception, no overlay leak, and no blocked retry.
- Physical Xiaomi/HyperOS duplicate-countdown verification was NOT TESTED in this milestone. No commit, push, merge, tag, or release was performed.

## 2026-09-17 (Human-like stress, root-cause regression, guard-release audit — APPAUSE-20260917-0002)
- Expanded the existing ADB stress harness with S13-S17: rapid Home/return, repeated Recents switching, lock/unlock return, mixed rotation/notification/app switching, and a near-countdown-end scenario that keeps the same target active while adding notification shade, rotation, and repeated same-target foreground noise.
- Added an integration-style JVM regression in `InterceptionDeciderTest`: a genuine initial open may intercept; after the old 30-second watchdog boundary an attached non-hideable 2032 still yields `PauseGuardPolicy.KEEP`; feeding that guard state back into `InterceptionDecider` must produce `SkipPauseShown` instead of a second intercept. Targeted `InterceptionDeciderTest` + `PauseGuardPolicyTest` PASS.
- Final near-end emulator run on API 37 `Medium_Phone` (`emulator-5554`) with a real 60-second Stress cooldown PASS. Evidence `scripts/stress/evidence/run-20260917-000356`: exactly 1 INTERCEPT and 1 type=2032 overlay; at about 55 seconds notification open/close, landscape rotation, and three repeated Clock launches produced only `SKIP: cooldown overlay is showing`; analyzer reported no duplicate interception, no overlay leak, and no blocked retry. The countdown completed normally and bypass then started.
- Earlier S13-S16 combined evidence `run-20260916-235018` produced an analyzer cross-scenario false positive between S14 and S15 because the harness force-stops/rebinds Appause between scenarios without an overlay-dismiss log from the old process. S15 rerun alone (`run-20260916-235244`) passed with 1 INTERCEPT / 1 overlay and no duplicate/leak. S16's explicit switch to an ordinary app legitimately abandons the current cooldown; a return may start a fresh cooldown and is not a duplicate-countdown failure.
- Audited every meaningful pause-guard clear/release path: `releasePauseGuard`, the `pauseShown` watchdog setter/getter paths, confirmed Home/SystemUI dismissal, `AbandonCooldown`, `OverlayManager.dismiss`, `PauseActivity.onDestroy`, and service destruction. No new unsafe release path was found. The existing redundant `pauseTargetPackage = null` after `overlayManager.dismiss()` is harmless and was left unchanged.
- Corrected one stale service comment that still described the old unconditional 30-second release semantics. Python stress-script syntax check, targeted JVM tests, `assembleDebug`, and `git diff --check` all PASS. Physical Xiaomi/HyperOS was NOT TESTED in this follow-up. No commit, push, merge, tag, release, or deploy was performed.

## 2026-09-17 (v0.5.41 released — pause-overlay stacking fix shipped)
- Bumped to `versionName 0.5.41` / `versionCode 93` (patch-only, per project convention) and published the GitHub Release `v0.5.41` with the signed APK `Appause-v0.5.41.apk`.
- Shipped the pause-guard fix from `8ce9240`: the 30-second watchdog no longer releases the logical guard while the primary 2032 overlay is still attached, and the 30-second hard cap now applies only to the hideable 2038 fallback. Before the fix, a cooldown longer than 30 seconds let a single foreground event from the target app (the app switching one of its own pages is enough — no user action needed) run a second INTERCEPT, attach a second overlay, and orphan the first window.
- Also shipped the docs/stress commit `d210197`: `docs/REPEAT_INTERCEPTION_STRESS_TEST.md` plus the ADB stress harness (`scripts/stress/{ui_stress,analyze,uinav}.py`). Evidence output is gitignored — it contains real-device package names and copies of the device database and persistent log.
- Verification for this release: `testDebugUnitTest` PASS with 22 suites / **216 tests, 0 failures, 0 errors, 0 skipped**; `assembleRelease` PASS (`BUILD SUCCESSFUL in 2m 39s`); `aapt2` reports `versionCode='93' versionName='0.5.41'` / `minSdk 26`; `apksigner verify` shows V2 signer `CN=Appause` with SHA-256 `843d4ce0…431525`, **byte-identical to the published v0.5.40 APK**, so in-place upgrade remains valid.
